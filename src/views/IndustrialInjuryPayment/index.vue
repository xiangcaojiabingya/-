<template>
  <!-- 工伤缴费历史查询 -->
  <div class="industrial_injury_payment">
    <!-- 顶部导航栏 -->
    <web-view-title title="工伤缴费历史查询"></web-view-title>
...
export default {
  ...
  methods: {
    ...
    toSearch(value) {
      ...
      this.list = [];
      this.getPayHarm(value.startMonth, value.endMonth);
    },
    //获取中台token
    getZTToken() {
      this.$toast.loading({
    ...
    //获取工伤缴费汇总查询
    getPayHarmTotal() {
      ajaxPost(apiUrl.getPayHarmTotal, { rsxtid: "3761" })
        .then((result) => {
          ...
          this.getPayHarm(getLastSixMonNum()[1], getLastSixMonNum()[0]);
        })
        .catch((error) => {
          console.log(error);
          this.getPayHarm(getLastSixMonNum()[1], getLastSixMonNum()[0]);
    ...
    //获取缴费列表
    getPayHarm(startDate, endDate) {
      ajaxPost(apiUrl.getPayHarm, {
        rsxtid: "3761",
        qsny: startDate,
        zzny: endDate,
      })
