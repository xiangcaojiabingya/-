...
export function axoisPostNormal(url, obj, header) {
    return new Promise((resolve, reject) => {
        request({
            ...
            if (result.data.responseData.status == '0') {
                resolve(result.data.responseData)
            } else if (result.data.responseData.status == '1') {
                //此处后台token失效重新调取登录接口
                Toast(result.data.responseData.message)
            } else {
                Toast("其他错误信息 " + result.data.responseData.message)
            }
